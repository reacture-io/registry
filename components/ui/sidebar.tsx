...
>(
  (
    ...
    const toggleSidebar = useCallback(() => {
      if (isMobile) {
        setOpenMobile((_openMobile) => !_openMobile);
      } else {
  ...
  (
    ...
    return (
      <div
        ...
        <div
          className={cn(
            'absolute inset-y-0 z-10 hidden h-svh w-[var(--sidebar-width)] transition-[left,right,width] duration-200 ease-linear md:flex',
            side === 'left'
...
const sidebarMenuSubButtonVariants = cva('cursor-pointer', {
  variants: {
    disabled: {
    ...
    return (
      <Comp
        ...
        className={cn(
          ...
          'group-data-[collapsible=icon]:hidden',
          className,
          sidebarMenuSubButtonVariants({ disabled })
        )}
        {...props}
      />
    );
